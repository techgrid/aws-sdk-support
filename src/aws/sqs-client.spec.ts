...
describe("SQSSupport", () => {
  ...
  it("should call error callback on send failure", async () => {
    ...
    sqsSupport = new SqsClient(new SQS());
    await sqsSupport.sendMessage(queue, message, null, callback);
    expect(callback).toBeCalledTimes(1);
  });
  ...
  it("should receive message from queue", async () => {
    ...
    sqsSupport = new SqsClient(new SQS());
    await sqsSupport.receiveMessage(queue, {timeout: 30}, callback);
    expect(callback).toBeCalledTimes(1);
  });
  ...
  it("should call error callback on receive failure", async () => {
    ...
    sqsSupport = new SqsClient(new SQS());
    await sqsSupport.receiveMessage(queue, {count: 2}, null, callback);
    expect(callback).toBeCalledTimes(1);
  });
});
